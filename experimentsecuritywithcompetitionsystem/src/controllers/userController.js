...
var validateFn=require('../middlewares/validationFn.js');
...
exports.processGetSubmissionData = async(req, res, next) => {
    ...
    try {
        ...
        if (results) {
            validateFn.sanitizeResult(results[0]);
            var jsonResult = {
...
exports.processGetSubmissionsbyEmail = async(req, res, next) => {
    ...
    if (role == 1) {
        ...
        return res.status(403).json({ message: 'Only admins can access this function' });
...
exports.processGetOneUserData = async(req, res, next) => {
    ...
    try {
        ...
        if (results) {
            validateFn.sanitizeResult(results);
            var jsonResult = {
                'userdata': results[0],
            }
            return res.status(200).json(jsonResult);
...
exports.processUpdateOneUser = async(req, res, next) => {
    ...
    if (role == 1) {
        ...
        return res.status(500).json({ message: 'Only admins can access this function' });
    }
